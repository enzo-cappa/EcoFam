class Period < ActiveRecord::Base
  ...
  scope :actual, -> {where(month: Time.now.month, year: Time.now.year).first_or_create}

  def next
    next_month = DateTime.new(self.year, self.month, 1) + 1.month
    Period.where(month: next_month.month, year: next_month.year).first_or_create
  end

  def prev
    prev_month = DateTime.new(self.year, self.month, 1) - 1.month
    Period.where(month: prev_month.month, year: prev_month.year).first_or_create
  end
end
