...
(function() {
  ...
  Tag = (function() {
    ...
    Tag.prototype.show = function() {
      ...
      that.element.wrap($('<div class="tags">')).parent().on('click', function() {
        return that.input.focus();
      });
      if (that.values.length) {
        $.each(that.values, function() {
          return that.createBadge(this);
        });
      }
      that.input = $('<input type="text">').attr('placeholder', that.options.placeholder).insertAfter(that.element).on('focus', function() {
        return that.element.parent().addClass('tags-hover');
      }).on('blur', function() {
        if (!that.skip) {
          that.process();
        ...
        if (event.keyCode === 188 || event.keyCode === 13 || event.keyCode === 9) {
          ...
          if (count) {
            ...
            if ((tag.hasClass('tag-important'))(that.remove(count - 1))) {
              ...
              return tag.addClass('tag-important');
            }
          }
        } else {
          return that.element.siblings('.tag').removeClass('tag-important');
        }
      }).typeahead({
        hint: true,
        highlight: true,
      ...
      }).on('typeahead:selected', function(event, suggestion, setName) {
        return that.add(suggestion.value);
    ...
    Tag.prototype.inValues = function(value) {
      var index;
    ...
    Tag.prototype.createBadge = function(value) {
      var that;
      that = this;
      return $('<span/>', {
        'class': "tag"
      }).text(value.toString()).append($('<button type="button" class="close glyphicon glyphicon-remove"></button>').on('click', function() {
        return that.remove(that.element.siblings('.tag').index($(this).closest('.tag')));
      })).insertBefore(that.element);
    };

    Tag.prototype.add = function(value) {
      var badge, index, that;
      that = this;
      if (!that.options.allowDuplicates) {
        ...
        if (index !== -1) {
          ...
          setTimeout(function() {
            return $(badge).removeClass('tag-warning');
          }, 500);
          return;
        }
      }
      ...
      return this.element.trigger('added', [value]);
    };

    Tag.prototype.remove = function(index) {
      var value;
      if (index >= 0) {
        value = this.values.splice(index, 1);
        this.element.siblings('.tag:eq(' + index + ')').remove();
        this.element.val(this.values.join(', '));
        return this.element.trigger('removed', [value]);
      }
    };

    Tag.prototype.process = function() {
      var that, values;
  ...
  $(window).on('load', function() {
    return $('[data-provide="tag"]').each(function() {
      var that;
      that = $(this);
      if (that.data('tag')) {
        return;
      }
      return that.tag(that.data());
    });
  });
...
(function() {
  var closeAlerts, extractDayNum;

  extractDayNum = function(row) {
    return $(row).find('td:first').text().match(/\d{2}/)[0].match(/\d{2}/)[0];
  };

  window.spendsSorter = function(rowA, rowB) {
    return extractDayNum(rowA) > extractDayNum(rowB);
  };

  window.updateNav = function(entityName) {
    $('ul.nav li.active').removeClass('active');
    return $('li:has(a#' + entityName + '-list-item)').addClass("active");
  };
